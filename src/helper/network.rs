...
pub async fn get_virtual_interfaces() -> Result<Vec<String>> {
    let mut interfaces = vec![];
    let mut entries = tokio::fs::read_dir(INTERFACE_VIRTUAL_DIR)
        .await
...
pub fn sync_get_virtual_interfaces() -> Result<Vec<String>> {
    let mut interfaces = vec![];
    let mut entries = std::fs::read_dir(INTERFACE_VIRTUAL_DIR).context(CommonIoSnafu)?;
